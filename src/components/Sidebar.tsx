  ...
  return (
    <aside className="hidden md:flex w-64 h-full bg-primary text-on-primary flex-col shrink-0 overflow-hidden">
      ...
      <nav className="flex-1 px-3 py-2 space-y-0.5 overflow-y-auto">
        ...
        <div className="flex items-center gap-3 px-4 py-3 text-on-primary/40 cursor-not-allowed">
      ...
      <div className="px-5 py-5 border-t border-white/10 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <Link href="/profile" className="p-1.5 hover:bg-white/10 rounded-lg transition-colors" title="Profile">
            <Icon name="manage_accounts" className="text-sm" />
          </Link>
          <button onClick={onLogout} className="p-1.5 hover:bg-white/10 rounded-lg transition-colors" title="Sign out">
    ...
    <>
      ...
      <aside className="fixed inset-y-0 left-0 z-50 w-72 bg-primary text-on-primary flex flex-col shadow-xl animate-slide-in">
        ...
        <nav className="flex-1 px-4 py-4 space-y-1 overflow-y-auto">
          ...
          <Link href="/badges" onClick={onClose} className="flex items-center gap-3 px-4 py-3 rounded-xl text-on-primary/80 hover:bg-primary-container/60 transition-colors">
        ...
        <div className="px-5 py-5 border-t border-white/10 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <Link href="/profile" onClick={onClose} className="p-1.5 hover:bg-white/10 rounded-lg transition-colors" title="Profile">
              <Icon name="manage_accounts" className="text-sm" />
            </Link>
            <button onClick={() => { onLogout(); onClose(); }} className="p-1.5 hover:bg-white/10 rounded-lg transition-colors" title="Sign out">
              <Icon name="logout" className="text-sm" />
            </button>
          </div>
        </div>
  ...
  return (
    <nav className="md:hidden bg-surface-container-lowest border-t border-outline-variant/20 flex shrink-0 pb-safe z-30">
      ...
      <label className={`flex-1 flex flex-col items-center gap-0.5 py-3 cursor-pointer transition-colors ${activeTab === "add" ? "text-primary" : "text-on-surface-variant"}`}>
        ...
        <input type="file" accept=".gpx" multiple onChange={(e) => { onFileUpload(e); onTabChange("routes"); }} className="hidden" />
      </label>
